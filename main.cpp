...
// 根据三元组 生成部队
// i win return true, j win return false
// 攻击序列i, i, j, j, i, i, j, j,.. 
bool fight_fromid(int i, int j, const vector<vector<int>>& ids, const Unit* baseunit, const std::map<std::string, bool> &skill_list){
...
int main(int argc, char **argv){
    ...
    vector<vector<int>> res_record(N, vector<int>(N, 0));  // 作战记录表

    for (int k = 0; k < FIGHT_NUM; ++k){
        cout << "cnt num = " << k << endl;
        for (int i = 0; i < N; ++i){
            for (int j = 0; j < N; ++j){
                bool res = fight_fromid(i, j, legions_id, fleet_data, skill_list);
                if (res){   // i->j win
                    res_record[i][j] += 1;
                }
                else{
