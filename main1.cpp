...
// A 先攻击， 根据行动力进行攻击
double attack(Legion& A, Legion& B, const std::map<std::string, bool> &skill_list, bool verbose = true){
    if (verbose){
        cout << "start..." << endl;
    }
...
int main(int argc, char **argv){
    ...
    ifstream f22("../data/unit/Hussar.json");
    ...
    int defence_num = unit_num1;
    Unit* attack_set = fleet_data1;
    Unit* defence_set = fleet_data1;

    // General* attack_gen = nullptr;
    General* attack_gen = gen0;

    // General* defence_gen = nullptr;
        ...
        for(int i = 0; i < attack_num; ++i){
            for (int j = 0; j < defence_num; ++j){
                ...
                for (int loop = 0; loop < LOOP_NUM; ++loop){
                    ...
                    record_sum1 += attack(A, B, skill_list, false);
                }
                // 调换攻守
                for (int loop = 0; loop < LOOP_NUM; ++loop){
                    Legion A("legion1");
                    ...
                    record_sum2 += attack(A, B, skill_list, false);
                }
                record1[i][j] = record_sum1 / LOOP_NUM;  
                record2[i][j] = record_sum2 / LOOP_NUM;              
            }
        }
    }
